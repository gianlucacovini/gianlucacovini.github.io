---
layout: archive
title: "Research"
description: "Publications, talks, and software by Gianluca Covini, PhD student in Statistics & Computer Science at Bocconi University, working on optimal transport and related topics."
permalink: /research/
author_profile: true
---

This page collects my publications and talks.

## Publications

- **Enhancing Parameter Control Policies with State Information**  
  **G. C.**, D. Antipov, C. Doerr  
  *FOGA '25 — Proceedings of the 18th ACM/SIGEVO Conference on
  Foundations of Genetic Algorithms*, Leiden, 2025, pp. 37–48.  
  [ACM DL](https://doi.org/10.1145/3729878.3746633){:target="_blank" rel="noopener"} ·
  [arXiv:2507.08368](https://arxiv.org/abs/2507.08368){:target="_blank" rel="noopener"}

## Talks

- **Dynamic Parameter Policies for LeadingOnes on Complex State Spaces**  
  *ROADEF — Congress of the French Operations Research Society*,
  Champs-sur-Marne, March 2025.  
  Main results of my MSc thesis, developed during a research stay at
  LIP6, Sorbonne Université/CNRS.  
  [Slides]({{ '/assets/files/Talks/Presentation_ROADEF.pdf' | relative_url }})

- **Variational Inference via Wasserstein Gradient Flows** (expository)  
  *Modern Methods in Applied Stochastics and Nonparametric Statistics*,
  WIAS Berlin (online), 2025.  
  On Lambert et al., *Variational Inference via Wasserstein Gradient Flows*.  
  [Slides]({{ '/assets/files/Talks/WIAS_PhD_presentation.pdf' | relative_url }})

## Software

- **SlicedWasserstein.jl** — sliced Wasserstein distances in Julia.  
  Registered in the Julia General Registry (v0.1.0).  
  [Docs](...){:target="_blank" rel="noopener"} ·
  [GitHub](...){:target="_blank" rel="noopener"} ·
  [doi:10.5281/zenodo.20631260](https://doi.org/10.5281/zenodo.20631260)){:target="_blank" rel="noopener"}
